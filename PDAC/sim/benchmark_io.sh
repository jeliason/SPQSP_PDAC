...
run_config() {
    ...
    echo "  [${label}] abm_out=${abm_out} pde_out=${pde_out} interval=${interval}"
...
# Run the 3 test configs for a given binary
run_suite() {
    local binary="$1"
    local suite_dir="$2"
    local suite_label="$3"
    ...
    echo ""
    echo "--- Running suite: ${suite_label} ---"
    echo "  Binary: ${binary}"

    run_config "$binary" "no_io"          0 0 1          "${suite_dir}/no_io"
    run_config "$binary" "io_every_step"  1 1 1          "${suite_dir}/io_every_step"
    run_config "$binary" "io_interval_${INTERVAL}" 1 1 "$INTERVAL" "${suite_dir}/io_interval_${INTERVAL}"
}
