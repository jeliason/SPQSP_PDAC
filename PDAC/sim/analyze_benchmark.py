...
def find_run_dirs(bench_dir):
    ...
    for d in sorted(bench_dir.iterdir()):
        if not d.is_dir():
            continue
        if (d / "outputs" / "timing.csv").exists():
            runs.append(d)
        else:
            # Check one level deeper (A/B suite layout)
            for dd in sorted(d.iterdir()):
                if dd.is_dir() and (dd / "outputs" / "timing.csv").exists():
                    runs.append(dd)
    return runs
...
def format_suite_table(suite_label, summaries):
    ...
    lines.append("")
    header = f"  {'Config':<25} {'Wall(s)':>8} {'Avg/step':>10} {'Min':>8} {'Max':>8} {'PDE':>8} {'QSP':>8} {'ABM*':>8}"
    lines.append(header)
    ...
    for s in summaries:
        ...
        lines.append(
            ...
            f"{s['min_step_ms']:>7.1f} {s['max_step_ms']:>7.1f} "
