...
/*! Check whether a specifc voxel is open to a specific type.
	\param[in] const Coord& c: the location to check
	\param[in] AgentType type: for each type "open" could be different.
	\return: true if the cell is open to this type.
*/
bool SpatialCompartment::voxelIsOpen(const Coord& c, AgentType type)const {
	return _agGrid.inGrid(c) && _agGrid(c)->isOpenToType(type);
}
...
bool SpatialCompartment::check_neighbor_agents(const CoordVec& env, const Coord& current,
	std::function<bool(const BaseAgent*)>f)const{ //bool foo(const BaseAgent* bar) {...}
