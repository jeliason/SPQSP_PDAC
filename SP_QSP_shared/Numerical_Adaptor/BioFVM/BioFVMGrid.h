...
public:
	...
	void update_source(double vol, double volV, double dt, double secretion, double saturate);
	//! update sink parameters
	void update_sink(double vol, double volV, double dt, double uptake);
	//! update sink/source parameters
	void update_sink_source(double vol, double volV, double dt, double secretion, double saturate, double uptake);
...
public:
	...
	//! get gradient at given dimension in voxel (x,y,z)
	std::vector< std::vector<double> > get_gradient(const Coord3D& c);
	//! get gradient at given dimension in voxel (voxel index i)
	std::vector< std::vector<double> > get_gradient(size_t n);
	...
	//! get number of voxel
	int get_num_voxel(void) const { return _nrVoxel; };
...
inline std::ostream& operator<<(std::ostream& os, const BioFVMGrid& g) {
	for (size_t i = 0; i < g.get_num_voxel(); i++) {
		Coord3D c = g.idx_to_coords(i);
		os << c.x << ',' << c.y << ',' << c.z << ",";
		auto delim = "";
		for (size_t j = 0; j < g.get_num_substrates(); j++) {
			os << delim << g(c, j);
			delim = ",";
		}
		os << std::endl;
