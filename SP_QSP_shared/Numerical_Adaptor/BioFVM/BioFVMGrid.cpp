...
/*! get gradient of(x,y,z); with given dimension d; x, y, z  are integer voxel coordinates
	...
	substrate i
*/
std::vector< std::vector<double> > BioFVMGrid::get_gradient(const Coord3D& c) {
	unsigned int idx = get_voxel_idx(c);
	//std::cout << "get gradient index: " << idx <<" , ( " << c.x << ", " << c.y << ", " << c.z << ") " << std::endl;
	return _tme.gradient_vector(idx);
}

std::vector< std::vector<double> > BioFVMGrid::get_gradient(size_t n) {
	return _tme.gradient_vector(n);
}
...
void BioFVMGrid::timestep(double dt){
	...
	source_sink_step(dt);
	_tme.simulate_diffusion_decay( dt );
	_tme.reset_all_gradient_vectors();
	//bool compute_gradient = true;
	//if (compute_gradient) {
	//_tme.compute_all_gradient_vectors();
	//}
#endif
}
...
void BioFVMSinkSource::update_source(double vol, double volV, double dt, double secretion, double saturate){
	update_sink_source(vol, volV, dt, secretion, saturate, 0);
}
...
void BioFVMSinkSource::update_sink_source(double vol, double volV, double dt, 
	double secretion, double saturation, double uptake){
#if SOURCE_SINK_SPLIT_SECOND_ORDER 
	set_internal_rates(vol, volV, dt/2, secretion, saturation, uptake);
#else
	set_internal_rates(vol, volV, dt, secretion, saturation, uptake);
#endif
}
...
void BioFVMSinkSource::set_internal_rates(double vol, double volV, double dt, 
	double secretion, double saturate, double uptake){
	//set_internal_rates_implicit(vol, volV, dt, secretion, saturate, uptake);
	//set_internal_rates_analytical(vol, volV, dt, secretion, saturate, uptake);
	set_internal_rates_analytical(volV, dt, secretion, uptake);
	return;
}
*/
...
/*! solve linear ODE
    Units of secretion and uptake are fractional concentration change rates within cell
    dp/dt = s*(T-p) - u*p
...
void BioFVMSinkSource::set_internal_rates(double vol, double volV, double dt, 
	...
	_solver_temp2 = secretion + uptake;
	...
	_solver_temp1 *= saturate* secretion / (secretion + uptake);
	return;
}

#else
/*! analytically solve source/sink step, no saturation
	Units of secretion: amounts per time.
